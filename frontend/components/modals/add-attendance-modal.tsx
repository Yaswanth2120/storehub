...
export function AddAttendanceModal({
  ...
  const {
    ...
    defaultValues: {
      ...
      totalHours: attendance?.totalHours ?? 40,
    },
  });
  ...
  useEffect(() => {
    setValue("weekStart", attendance?.weekStart ?? weekStart, { shouldValidate: true });
  }, [attendance?.weekStart, setValue, weekStart]);

  const storeId = watch("storeId");
  const workerType = watch("workerType");
  const filteredWorkers = workers.filter((worker) => worker.storeIds.includes(storeId) && worker.workerType === workerType);
  const isManagerWeekAllowed = role !== "MANAGER" || allowedWeekStarts.includes(weekStart);
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    toast.success(attendance ? "Weekly attendance updated" : "Weekly attendance saved");
    setOpen(false);
    router.refresh();
  });
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant={attendance ? "outline" : "default"} size={attendance ? "sm" : "default"}>
          {triggerLabel}
        </Button>
      </DialogTrigger>
      <DialogContent className="rounded-[28px] border-[#e5e5ea] bg-white">
        <DialogHeader>
          <DialogTitle className="text-[1.65rem] font-semibold tracking-[-0.03em] text-[#1d1d1f]">
            {attendance ? "Update weekly attendance" : "Add weekly attendance"}
          </DialogTitle>
          <DialogDescription className="text-[14px] leading-6 text-[#6e6e73]">
            Managers enter the total weekly hours worked for each employee or manager within the selected Saturday to Friday period.
          </DialogDescription>
        </DialogHeader>

        <form className="space-y-4" onSubmit={onSubmit}>
          <div className="rounded-[24px] border border-[#e5e5ea] bg-[#f8f8fa] px-4 py-3 text-sm text-[#4b4b51]">
            Selected week: <span className="font-medium text-[#1d1d1f]">{weekStart}</span> to{" "}
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium text-[#3a3a3c]">Worker type</label>
            <Select
              value={workerType}
              onValueChange={(value) => setValue("workerType", value as "EMPLOYEE" | "MANAGER", { shouldValidate: true })}
            >
              <SelectTrigger className="h-12 rounded-2xl border-[#d2d2d7] bg-[#fbfbfd] shadow-none">
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium text-[#3a3a3c]">{workerType === "MANAGER" ? "Manager" : "Employee"}</label>
            <Select value={watch("workerId")} onValueChange={(value) => setValue("workerId", value, { shouldValidate: true })}>
              <SelectTrigger className="h-12 rounded-2xl border-[#d2d2d7] bg-[#fbfbfd] shadow-none">
                <SelectValue placeholder={`Select ${workerType === "MANAGER" ? "manager" : "employee"}`} />
              </SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium text-[#3a3a3c]">Total weekly hours</label>
            <Input
              type="number"
              min={0.1}
              step="0.01"
              ...
              {...register("totalHours")}
            />
            {errors.totalHours ? <p className="text-xs text-destructive">{errors.totalHours.message}</p> : null}
          </div>

          <DialogFooter>
            <Button type="submit" disabled={loading || !isManagerWeekAllowed}>
              {loading ? "Saving..." : attendance ? "Save changes" : "Save weekly hours"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
